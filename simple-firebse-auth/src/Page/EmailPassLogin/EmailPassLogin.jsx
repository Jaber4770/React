...
const EmailPassLogin = () => {
    ...
    const handleLogin = (e) => {
        ...
        const termsAccpt = e.target.terms.checked;
        console.log(email, password, termsAccpt);
        ...
        createUserWithEmailAndPassword(auth, email, password)
            .then(userCredential => {
                if (!termsAccpt) {
                    setErrMsg("please accept the trams and conditions!");
                    return;
                }
                ...
                sendEmailVerification(auth.currentUser)
                    .then(() => {
                        setEmailVerification("Please check your email to verify!");
                    })
            })
            .catch(error => {
    ...
    return (
        <form className='border-1 border-white p-5 mx-5' onSubmit={handleLogin}>
            ...
            <label className="label py-5">
                ...
                Accept the trams and conditions.
            </label>
            <br />
            ...
            <div>
                ...
                {
                    emailVerification && <p className='text-center text-green-500'>{emailVerification}</p>
                }
            </div>
        </form>
    );
